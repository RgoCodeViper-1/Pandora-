...
class WhisperManager:
    ...
    def _resolve_model_path() -> Path:
        """Select the preferred installed English model."""
        models_dir = WHISPER_DIR / "models"
        preferred = models_dir / "ggml-base.en.bin"
        if preferred.exists():
            return preferred

        quantized = sorted(models_dir.glob("ggml-base.en-*.bin"))
        if quantized:
            return quantized[0]
    ...
    def ensure_running(self) -> bool:
        ...
        4. Wait up to 45 s for the server to accept connections.
        ...
        # Model loading can take substantially longer than process startup.
        ...
        if self._process is not None and self._process.poll() is not None:
            logger.error(
                "whisper.cpp server exited during startup (exit code %s)",
                self._process.returncode,
            )
        else:
            logger.error(
                "whisper.cpp server did not become ready within %.0f s",
                _READY_TIMEOUT,
            )
        return False
    ...
    @staticmethod
    def _log_process_stderr(process: subprocess.Popen) -> None:  # type: ignore[type-arg]
        """Forward server diagnostics without blocking the manager."""
        if process.stderr is None:
            return
        for line in process.stderr:
            message = line.strip()
            if message:
                logger.debug("whisper-server: %s", message)

...
try:
    ...
    logger.warning("TTSService not available — using console responses: %s", exc)
...
def speak(text: str, priority: bool = False) -> None:
    ...
    if not text:
        return
    if _svc is None:
        # Keep the assistant observable when optional audio dependencies are
        # unavailable; the response remains useful in a terminal session.
        print(f"[PANDORA] {text}", flush=True)
        return
...
def interrupt() -> None:
    """Stop active TTS playback when the user starts speaking."""
    if _svc is None:
        return
    try:
        running_loop = asyncio.get_running_loop()
        running_loop.create_task(_svc.interrupt())
    except RuntimeError:
        loop = _ensure_background_loop()
        asyncio.run_coroutine_threadsafe(_svc.interrupt(), loop)
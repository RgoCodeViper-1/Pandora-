...
class SpeechToTextEngine:
    ...
    def transcribe_pcm(self, pcm_bytes: bytes, sample_rate: int = 16000) -> str:
        ...
        cache_key = cache.make_key("stt", str(sample_rate), pcm_bytes)
        ...
        if self._whisper is not None:
            ...
            if text:
                cache.set(cache_key, text, ttl_seconds=7 * 24 * 60 * 60)
                return text
        ...
        if self._google is not None:
            ...
            try:
                with wave.open(path, "wb") as wf:
                    ...
                    wf.setframerate(sample_rate)
                    wf.writeframes(pcm_bytes)
                text = self._google.transcribe_wav(path)
                if text:
                    cache.set(cache_key, text, ttl_seconds=7 * 24 * 60 * 60)
                return text
            finally:
    ...
    def transcribe_wav(self, wav_path: str) -> str:
        ...
        try:
            ...
            logger.warning("Unable to read WAV for STT: %s", exc)
            return ""

        cache_key = cache.make_key("stt", audio_bytes)
        cached = cache.get(cache_key)
        if isinstance(cached, str):
            logger.debug("STT cache hit")
            return cached
        ...
        if self._whisper is not None:
            ...
            if text:
                ...
                return text
        if self._google is not None:
            text = self._google.transcribe_wav(wav_path)
            if text:
                cache.set(cache_key, text, ttl_seconds=7 * 24 * 60 * 60)
            return text
        return ""

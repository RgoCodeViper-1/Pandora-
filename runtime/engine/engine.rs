...
// ─────────────────────────────────────────────────────────────────────────────
// Lua config loader — module-level so no Self:: qualification needed
// ─────────────────────────────────────────────────────────────────────────────

/// Compile-time embedded fallback.
/// Path from src/engine.rs: src/ → core_engine/ → rust/ → PANDORA/
/// So three levels up to reach PANDORA/runtime/engine/engine_fields.lua
const DEFAULT_LUA: &str =
    include_str!("../../../runtime/engine/engine_fields.lua");

/// Resolve the live Lua config path at runtime.
/// Three-layer strategy: env var → directory walk → bare relative path.
fn lua_config_path() -> std::path::PathBuf {
    ...
    // Handles `cargo test` (cwd = crate root) and direct invocation equally.
    ...
    // Layer 3 — last resort: bare relative path (works when cwd == PANDORA/)
    std::path::PathBuf::from("runtime/engine/engine_fields.lua")
}

/// Load Pandora runtime knobs from engine_fields.lua.
///
/// Fallback chain:
///   live file on disk  →  compile-time embed (DEFAULT_LUA)  →  Rust hardcoded values
///
/// Each key is overlaid independently — one bad value never affects the others.
fn load_lua_config() -> (f32, bool, usize, Option<String>, bool, f32) {
    // Hardcoded Rust defaults — innermost failsafe, always valid
    ...
    // Read live file; on failure fall back to the compile-time embed
        ...
        Err(_) => {
            eprintln!(
                "[pandora_core] engine_fields.lua not found on disk — \
                 falling back to embedded compile-time copy"
            );
            DEFAULT_LUA.to_string()
        }
    };

    // Parse — on failure (malformed file) drop straight to Rust defaults
    let lua   = Lua::new();
    ...
    // Overlay each key independently.
    // Syntax: get::<_, TargetType>(key_str)
    //   first generic  = key type   → inferred from the &str argument
    //   second generic = value type → must be explicit
    // Lua floats are f64 natively; cast to f32 after reading.
    // Lua integers are i64 natively; cast to usize after reading.
    if let Ok(v) = table.get::<_, f64>("min_confidence")    { min_confidence   = v as f32; }
    if let Ok(v) = table.get::<_, bool>("multi_intent")     { multi_intent     = v; }
    if let Ok(v) = table.get::<_, i64>("max_intents")       { max_intents      = v as usize; }
    if let Ok(v) = table.get::<_, String>("wakeword")       { wakeword         = Some(v); }
    if let Ok(v) = table.get::<_, bool>("strip_wakeword")   { strip_wakeword   = v; }
    if let Ok(v) = table.get::<_, f64>("stream_threshold")  { stream_threshold = v as f32; }
...
impl EngineRuntime {
    ...
    ///
    /// Runtime knobs are loaded from `engine_fields.lua` via `load_lua_config()`.
    /// Patterns that fail to compile are logged and skipped — a single bad
    /// regex cannot crash the runtime.
    pub fn build() -> Self {
        // Load Lua config first — values feed directly into the struct below
        let (min_confidence, multi_intent, max_intents,
             wakeword, strip_wakeword, stream_threshold) = load_lua_config();
